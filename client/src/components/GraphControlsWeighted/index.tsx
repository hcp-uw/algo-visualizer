...
import "./GraphControlsWeighted.css";
...
const GraphControlsWeighted = ({
     ...
     * @param {string} weight
     */
    const addEdgeWeight = (n1: string, n2: string, weight: number | undefined) => {
      // check if trying to add edge to the same node
      if (n1 === n2) return;
      // check if edge already exist
      let newEdges = []
      ...
      setEdges([...newEdges, { n1, n2, weight }]);
    };
...
*/
    ...
    */
    ...
    /*
    Here we will be adding in the edgeWeight
    */
    const changeEdgeWeight = (elementStart: HTMLSelectElement, elementEnd: HTMLSelectElement, weight: number) => {
      let firstElement = elementStart.firstChild as HTMLOptionElement;
      let firstVal = firstElement.textContent;
      elementStart.innerHTML = "";
      let firstOption = document.createElement("option");
      firstOption.textContent = firstVal;
      let currentElement = elementStart;
      currentElement.appendChild(firstOption);

      let secondElement = elementEnd.firstChild as HTMLOptionElement;
      let secondVal = secondElement.textContent;
      elementEnd.innerHTML = "";
      let secondOption = document.createElement("option");
      secondOption.textContent = secondVal;
      let currentElement2 = elementStart;
      currentElement2.appendChild(secondOption);
  ...
  return (
    <>
      <div id="body">
        ...
        <div id = "addEdgeWeightPortion" className="dfs-graph-controls">
          <select id="addEdgeWeightStart" className="select_two"
            onMouseDown={() => {
            let element = document.getElementById("removeEdgesStart");
            getNodes(element as HTMLSelectElement)
          }}
          ...
          <select id="addEdgeWeightEnd" className="select_two"
            onMouseDown={() => {
              let element = document.getElementById("removeEdgesEnd");
              getNodes(element as HTMLSelectElement)
            }}
          ...
          <input id="nodeWeight" type="text" placeholder = "Input node Val"></input>
          <button className = "buttonClass"
            onClick={() => {
              let start = document.getElementById("removeEdgesStart") as HTMLSelectElement;
              let end = document.getElementById("removeEdgesEnd") as HTMLSelectElement;
              let weight = parseInt((document.getElementById("nodeWeight") as HTMLInputElement).value)
              changeEdgeWeight(start, end, weight)
            }}
          >
            Add Edge Weight
          </button>
...
export default GraphControlsWeighted;
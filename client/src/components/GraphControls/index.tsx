...
const validateNode = (
	...
	if (value === "") {
		return -1;
	} else if (Number.isNaN(integerValue)) {
		return -2;
	} else {
		return value;
	}
...
const GraphControls = ({
	...
	const addNode = (initX: number, initY: number) => {
		...
		// TODO: attempt
		let integerValue = validateNode(value);
		if (integerValue === -1) {
		...
		<>
			<div id="body">
				<div id="addNodePortion" className="dfs-graph-controls">
					...
					{/* <input id="addStartNode" type="text" placeholder="Start node Val"></input>
					<input id="addEndNode" type="text" placeholder="End node Val"></input>
					<button className="buttonClass"
						onClick={() => {
							let start = document.getElementById("addStartNode") as HTMLInputElement;
							let end = document.getElementById("addEndNode") as HTMLInputElement;
							// addNode(700, 225);
							addEdge(
								start.value,
								end.value,
